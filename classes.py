...
class DataBase:
    ...
    def createBookTable():
        ...
        cursor = conn.cursor()
        cursor.execute("create table %s (ID int primary key, name text, authors text, pub_date text, publisher text, isbn text, language text, cover_path text, rating int, file_path text, tags text, booklists text)" % DataBase.book_table)
        conn.commit()
        conn.close()
    ...
    def addBook(book):
        conn = sqlite3.connect(DataBase.db_name)
        cursor = conn.cursor()
        cursor.execute("insert into %s values(%s, '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s' ,'%s')" % (DataBase.book_table, book.ID, book.name, listToString(book.authors), book.pub_date, book.publisher,
